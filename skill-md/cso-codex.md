...
# cso - Codex Skill Package

## What This Is

This is a friend-safe Markdown copy of `cso` for Codex. It removes local filesystem paths, Finder-only links, and machine-specific source locations.
...
In Codex, click the chat box, press /, choose cso, then write the task. Fallback prompt: Use the cso skill to...
...
greps below target (AKIA, [github-token-prefix-redacted], sk-ant-, sk_live_, xoxb-, `-----BEGIN ... PRIVATE
...
git log -p --all -G "[github-token-prefix-redacted]|gho_|github_pat_" 2>/dev/null
...
**Severity:** CRITICAL for active secret patterns in git history (AKIA, sk_live_, [github-token-prefix-redacted], xoxb-). HIGH for .env tracked by git, CI configs with inline credentials. MEDIUM for suspicious .env.example values.
