...
display_name: design-taste-frontend
platform: Codex
category: Design, creative, and media
---

# design-taste-frontend - Codex Skill Package

## What This Is

This is a friend-safe Markdown copy of `design-taste-frontend` for Codex. It removes local filesystem paths, Finder-only links, and machine-specific source locations.
...
In Codex, click the chat box, press /, choose design-taste-frontend, then write the task. Fallback prompt: Use the design-taste-frontend skill to...

## Skill Metadata

| Field | Value |
| --- | --- |
| Display name | `design-taste-frontend` |
| Canonical name | `design-taste-frontend` |
| Platform | `Codex` |
| Category | Design, creative, and media |
...
---
name: design-taste-frontend
description: Cross-platform anti-slop app design skill for web frontends, dashboards, macOS, iOS, Android and Kotlin, React Native, Tauri, Electron, and Vercel Labs Native SDK. Reads the brief, selects the platform-native design language, routes to the right specialist, and ships accessible interfaces that feel intentional instead of templated.
---

# tasteskill: Anti-Slop Cross-Platform App Design

> Install name stays `design-taste-frontend`; its design scope covers web, mobile, and desktop apps.
> Landing pages, product UI, dashboards, data-heavy workflows, native apps, and redesigns are all in scope.
> Every rule below is **contextual**. None of it fires automatically. First read the brief, then pull only what fits.
> Platform conventions beat visual novelty. Web-specific rules apply only to web renderers.

Use `hallmark` as the shared anti-slop structure, study, redesign, and visual-audit layer when deeper art direction is useful. Load its platform adapter first for native apps; this skill still chooses the platform route, and the matching specialist remains authoritative for native behavior, accessibility, security, and verification.
...
1. **Platform** - web, macOS, iOS/iPadOS, Android/Kotlin, Compose Multiplatform, React Native, Tauri, Electron, or Native SDK. Detect it from the repository before choosing components.
2. **Surface kind** - landing, portfolio, dashboard, content browser, editor, settings, onboarding, multi-step workflow, data table, utility, menu-bar app, document app, or redesign.
3. **Vibe words** the user used - "minimalist", "calm", "Linear-style", "Awwwards", "brutalist", "premium consumer", "Apple-y", "playful", "serious B2B", "editorial", "agency-y", "glassy", "dark tech".
4. **Reference signals** - URLs they linked, screenshots they pasted, products they named, brands they're competing with.
5. **Audience and input model** - touch, keyboard, pointer, stylus, game controller, assistive technology, or a combination. The audience and device pick the interaction model, not your taste.
6. **Brand assets that already exist** - logo, color, type, photography. For redesigns, these are starting material, not optional input (see Section 11).
7. **Quiet constraints** - accessibility-first audiences, public-sector, regulated industries, trust-first commerce, kids' products, offline use, permissions, small windows, foldables, and large text. These constraints OVERRIDE aesthetic preference.

### 0.B Output a one-line "Design Read" before generating
Before any code, state in one line: **"Reading this as: \<surface> for \<audience> on \<platform>, with a \<vibe> language, leaning toward \<native system or aesthetic family>."**

Example reads:
- *"Reading this as: B2B SaaS landing for technical buyers, with a Linear-style minimalist language, leaning toward Tailwind utilities + Geist + restrained motion."*
- *"Reading this as: solo designer portfolio for hiring managers, with an editorial / kinetic-type language, leaning toward native CSS + scroll-driven animation + custom typography."*
- *"Reading this as: redesign of a public-sector service site, with a trust-first language, leaning toward GOV.UK Frontend or USWDS."*
- *"Reading this as: task-focused iPhone finance app for everyday consumers, with a calm trust-first language, leaning toward SwiftUI and Apple HIG patterns with restrained motion."*
- *"Reading this as: dense macOS research utility for keyboard-first professionals, with a native productivity language, leaning toward SwiftUI plus AppKit interop, sidebar navigation, menus, and shortcuts."*
- *"Reading this as: adaptive Android operations dashboard for field teams, with a practical Material 3 language, leaning toward Compose, window-size classes, and list-detail layouts."*
...
| "product app / dashboard / operational workflow" | 4-7 | 2-5 | 5-8 |
| "native utility / settings / document app" | 3-6 | 2-4 | 4-7 |
...
| Dashboard / admin / data tool | 5 | 3 | 7 |
| iOS or Android consumer app | 6 | 4 | 5 |
| macOS or desktop productivity app | 5 | 3 | 7 |
| Cross-platform desktop shell | 5 | 3 | 6 |
...
### 2.B Platform-native foundations

| Target | Default foundation | Non-negotiable character |
|---|---|---|
| iOS / iPadOS | SwiftUI, Apple HIG, SF Symbols, system materials and controls | Dynamic Type, safe areas, native navigation and modality, VoiceOver |
| macOS | SwiftUI plus AppKit interop when needed, Apple HIG, SF Symbols | Menus, keyboard shortcuts, resizable windows, sidebars/toolbars, pointer and focus behavior |
| Android / Kotlin | Jetpack Compose, Material 3, Material 3 Adaptive | Window-size classes, predictive/adaptive navigation, 48dp targets, TalkBack semantics |
| Kotlin Multiplatform / Compose Desktop | Compose Multiplatform with platform-specific adapters | Shared tokens are fine; navigation, menus, shortcuts, windowing, and permissions remain platform-specific |
| React Native / Expo | React Native primitives and navigation with deliberate `.ios` / `.android` divergence | Safe-area context, font scaling, VoiceOver/TalkBack, native gestures and platform conventions |
| Tauri | Existing web design system inside a secure Tauri shell | Desktop window states, keyboard/pointer input, native menus/dialogs where appropriate, scoped capabilities |
| Electron | Existing web design system inside a hardened Electron renderer | Native menus and accelerators, desktop window conventions, accessibility tree, secure IPC boundaries |
| Vercel Labs Native SDK | `native-sdk` workflow and the SDK's supported native primitives | Respect the SDK's real component and platform surface; do not invent browser APIs or unsupported controls |

**Platform truth rule:** shared brand tokens may travel across targets; interaction patterns do not travel unchanged. A cross-platform app may share color, type, spacing, and content hierarchy while using platform-specific navigation, controls, menus, permissions, and gestures.

**Honesty rule:** if the brief reads as one of the systems above, install and use the **official** package or platform framework. Do not recreate native controls with painted rectangles. Do not import a system's tokens but then override 90% of them.

**One coherent system per target.** Do not mix Fluent React with Carbon in the same web tree. Do not import shadcn/ui components into a native Material 3 or SwiftUI surface. Cross-platform products may map one brand system onto each target's native components.

### 2.C When the brief is an aesthetic, not a system
...
### 3.0 Select the implementation route before styling

Use this skill as the taste and design-direction layer. Pair it with the installed implementation specialist whenever one matches:

| Repository or target | Route implementation through |
|---|---|
| SwiftUI on iOS / iPadOS | `swiftui-master`, `swiftui-expert-skill`, and `mobile-ui-accessibility` |
| SwiftUI / AppKit on macOS | `build-macos-apps:swiftui-patterns`, `build-macos-apps:appkit-interop`, and `swiftui-master` |
| Android / Kotlin / Jetpack Compose | `android-jetpack-compose-expert` and `mobile-ui-accessibility` |
| React Native / Expo | `react-native-master` and `mobile-ui-accessibility` |
| Tauri | `tauri-desktop-framework` plus the web specialist matching the renderer |
| Electron | `electron-full-stack-developer` plus the web specialist matching the renderer |
| Vercel Labs Native SDK | `native-sdk` |
| Web / React / Next.js | the web rules below plus `build-web-apps:frontend-app-builder` when implementation is requested |

If a named specialist is unavailable, follow the same platform contract directly and verify against official platform documentation. Do not block solely because a companion skill is missing.

Sections 3.A through 3.F are the **web-renderer route**. Apply them to websites and the renderer layer of Tauri/Electron, not blindly to SwiftUI, Compose, or React Native.
...
For SwiftPM, Gradle, Cargo, or Native SDK projects, inspect the platform's real dependency manifest instead. Never emit an npm command for a native target.

### 3.G Shared app-design contract

These rules apply to every platform:

* **Task clarity before decoration:** identify the primary task, destructive actions, irreversible actions, and the shortest successful path.
* **Native hierarchy:** use the platform's navigation, toolbar, menu, dialog, sheet, back behavior, and selection conventions before inventing custom chrome.
* **Adaptive layout:** test compact and expanded widths, split view or multitasking, rotation where supported, window resizing, large text, and content growth. Never treat one screenshot size as the product.
* **Input completeness:** touch targets, hover, keyboard focus, shortcuts, pointer precision, stylus, and screen-reader traversal must match supported devices.
* **Accessibility:** preserve semantic roles, labels, values, state, traversal order, scalable text, contrast, reduce motion, increased contrast, and non-color status cues.
* **State completeness:** design loading, empty, error, offline, permission-denied, destructive confirmation, partial-success, and recovery states.
* **Data density:** dashboards and tables are in scope. Use hierarchy, alignment, sorting/filtering affordances, progressive disclosure, and readable density instead of wrapping every datum in a card.
* **System components first:** customize tokens and composition before replacing a platform control. Custom controls inherit the burden for semantics, focus, input, localization, and state.
* **Security is part of UX:** permission prompts, file access, external links, clipboard, IPC, and destructive actions need clear intent and least privilege.
* **Real verification:** use previews, simulators/emulators, headed desktop runs, accessibility inspection, and representative device/window sizes for the selected platform.

---

## 4. DESIGN ENGINEERING DIRECTIVES (Bias Correction)

**Applicability gate for Sections 4-12:** these sections preserve the upstream skill's deep web and marketing-page craft. Apply conceptual guidance such as hierarchy, typography, color, material, motion restraint, accessibility, and anti-template discipline everywhere. Exact Tailwind classes, CSS mechanics, React state advice, GSAP/Motion skeletons, hero formulas, page-section bans, SEO rules, and the block library are web-only. For native and product surfaces, translate the intent into the selected platform's components and ignore rules that describe a different surface.
...
* **Delivery baseline** - on web, capture ranking pages, meta titles, structured data, and OG cards because SEO migration is a major redesign risk. In apps, capture navigation routes, deep links, saved data compatibility, analytics events, permissions, keyboard shortcuts, and platform-specific user habits.
...
This block library is for the web and web-renderer route. Native patterns belong in platform-native components, previews, and the matching specialist skill rather than in these Tailwind/React block files.
...
This skill provides design direction for any app surface, including dashboards, tables, multi-step workflows, editors, native mobile, desktop utilities, and collaborative UI. It does not replace domain engineering.

Route these concerns to the appropriate specialist while retaining this skill's taste, hierarchy, accessibility, and pre-flight layers:

* Data-grid behavior, virtualization, spreadsheet semantics, or formula engines.
* Realtime collaboration protocols, presence synchronization, CRDTs, or operational transforms.
* Native persistence, background execution, signing, entitlements, packaging, and store submission.
* Tauri/Electron IPC implementation, capability configuration, process isolation, and updater security.
* Business rules, authentication, cryptography, financial calculations, medical logic, or regulatory compliance.

Do not respond to an in-scope app-design request with "this skill is only for landing pages." Select the platform route, apply the shared design contract, and involve the matching engineering skill.

---

## 14. FINAL PRE-FLIGHT CHECK

Run this matrix before outputting designs or code. This is the last filter.

**THIS IS NOT OPTIONAL. Run every universal box, every box for the selected platform, and every applicable surface-specific box. Web-only boxes do not apply to native views unless the target embeds a web renderer.**

### 14.A Universal app checks

- [ ] **Platform and surface declared** in the Design Read, based on the repository and brief?
- [ ] **Primary user task** and shortest successful path are visually obvious?
- [ ] **Platform-native foundation** selected before custom styling?
- [ ] **Adaptive states** checked at compact and expanded widths, large text, content growth, and supported orientation/window modes?
- [ ] **Input modes** verified for every supported device: touch, keyboard, pointer, stylus, or assistive technology?
- [ ] **Semantic accessibility** includes labels, roles, values, states, traversal/focus order, non-color cues, contrast, and reduced motion?
- [ ] **Loading, empty, error, offline, permission, destructive, partial-success, and recovery states** are designed where relevant?
- [ ] **One coherent token system** governs color, typography, spacing, radii, elevation/material, and motion across the target?
- [ ] **Custom controls justified** and tested against the platform control they replace?
- [ ] **Real platform verification** performed in the browser, preview, simulator/emulator, or headed desktop app rather than from static code alone?

### 14.B Platform checks

- [ ] **iOS/iPadOS:** Dynamic Type, safe areas, VoiceOver, native back/modality, tab/sidebar adaptation, SF Symbols, and touch targets verified?
- [ ] **macOS:** resizable windows, menus, keyboard shortcuts, focus, toolbar/sidebar behavior, pointer states, and VoiceOver verified?
- [ ] **Android/Kotlin:** Material 3 semantics, window-size classes, adaptive navigation, TalkBack, font scaling, predictive back, and 48dp targets verified?
- [ ] **React Native:** safe-area context, font scaling, VoiceOver/TalkBack, keyboard avoidance, platform-specific components, and native gestures verified on both requested platforms?
- [ ] **Tauri:** renderer responsiveness plus desktop menus/shortcuts/window states, least-privilege capabilities, and native dialogs verified?
- [ ] **Electron:** renderer accessibility plus native menus/accelerators/window conventions, context isolation, sandboxing, CSP, and validated IPC verified?
- [ ] **Native SDK:** components and behaviors exist in the installed SDK version, with supported windowing, input, accessibility, and packaging paths verified?

### 14.C Web and web-renderer checks
...
If a single applicable checkbox cannot be honestly ticked, the surface is not done. Fix it before delivering.
...
### Cross-platform app foundations
- Apple Human Interface Guidelines: https://developer.apple.com/design/human-interface-guidelines/
- Apple accessibility: https://developer.apple.com/design/human-interface-guidelines/accessibility/
- SwiftUI accessibility fundamentals: https://developer.apple.com/documentation/swiftui/accessibility-fundamentals
- Android adaptive Compose apps: https://developer.android.com/develop/ui/compose/build-adaptive-apps
...
- Vercel Labs Native SDK source and skill: https://github.com/vercel-labs/native
