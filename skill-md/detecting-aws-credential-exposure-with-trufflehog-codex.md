...
display_name: detecting-aws-credential-exposure-with-trufflehog
platform: Codex
category: Security, forensics, and incident response
---

# detecting-aws-credential-exposure-with-trufflehog - Codex Skill Package

## What This Is

This is a friend-safe Markdown copy of `detecting-aws-credential-exposure-with-trufflehog` for Codex. It removes local filesystem paths, Finder-only links, and machine-specific source locations.
...
In Codex, click the chat box, press /, choose detecting-aws-credential-exposure-with-trufflehog, then write the task. Fallback prompt: Use the detecting-aws-credential-exposure-with-trufflehog skill to...

## Skill Metadata

| Field | Value |
| --- | --- |
| Display name | `detecting-aws-credential-exposure-with-trufflehog` |
| Canonical name | `detecting-aws-credential-exposure-with-trufflehog` |
| Platform | `Codex` |
| Category | Security, forensics, and incident response |
...
trufflehog git [local file URL removed] --only-verified --json > trufflehog-results.json
...
secrets_scan:
  ...
  script:
    - trufflehog git [file URL removed]$CI_PROJECT_DIR --since-commit $CI_COMMIT_BEFORE_SHA --only-verified --fail
  allow_failure: false
```
