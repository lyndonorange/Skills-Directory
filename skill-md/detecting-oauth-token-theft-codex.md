...
display_name: detecting-oauth-token-theft
platform: Codex
category: Security, forensics, and incident response
---

# detecting-oauth-token-theft - Codex Skill Package

## What This Is

This is a friend-safe Markdown copy of `detecting-oauth-token-theft` for Codex. It removes local filesystem paths, Finder-only links, and machine-specific source locations.
...
In Codex, click the chat box, press /, choose detecting-oauth-token-theft, then write the task. Fallback prompt: Use the detecting-oauth-token-theft skill to...

## Skill Metadata

| Field | Value |
| --- | --- |
| Display name | `detecting-oauth-token-theft` |
| Canonical name | `detecting-oauth-token-theft` |
| Platform | `Codex` |
| Category | Security, forensics, and incident response |
