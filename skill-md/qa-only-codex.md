...
# qa-only - Codex Skill Package

## What This Is

This is a friend-safe Markdown copy of `qa-only` for Codex. It removes local filesystem paths, Finder-only links, and machine-specific source locations.
...
In Codex, click the chat box, press /, choose qa-only, then write the task. Fallback prompt: Use the qa-only skill to...

## Skill Metadata

| Field | Value |
| --- | --- |
| Display name | `qa-only` |
| Canonical name | `qa-only` |
| Platform | `Codex` |
| Category | General and specialized workflows |
